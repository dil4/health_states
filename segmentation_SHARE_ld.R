...
if (!require("msm")) install.packages("msm") # Installs msm if needed
...
#table(dat_SHARE$ph065_)
#TABLE
#-2   -1    1     5 
#1    6     503   1886

dat_SHARE$a<-ifelse(dat_SHARE$ph065_==5, 1,0)
#table(dat_SHARE$a)
#a=1  No weight loss
#a=0  Weight loss
#     & Don't know + Refusal
...
list(dat_SHARE$ph095_)
#VALUE      LABEL
#   -2      Refusal
#   -1      Don't know

#table(dat_SHARE$ph095_)
#TABLE
...
dat_SHARE$b<-ifelse(dat_SHARE$ph095_<5,1,0)
#table(dat_SHARE$b)
...
#table(dat_SHARE$ph066_)
#TABLE
#  1   2   3  97 
# 91 143  23 246

dat_SHARE$c<-ifelse((dat_SHARE$ph066_==2|dat_SHARE$ph066_==3),1,0)
#table(dat_SHARE$c)
...
#table(dat_SHARE$ph089d1)
#TABLE
#    -2   -1    0    1 
#     2    2 2243  143

dat_SHARE$d<-ifelse(dat_SHARE$ph089d1==1,1,0)
#table(dat_SHARE$d)
...
#list(dat_SHARE$ph006d1:ph006d21)
#  value  label
#-2      Refusal
#-1   Don't know
# 0 Not selected
...
dat_SHARE$e<-ifelse( (dat_SHARE$ph006d1==1
                      |dat_SHARE$ph006d2==1
                      |dat_SHARE$ph006d3==1
                      |dat_SHARE$ph006d4==1
                      |dat_SHARE$ph006d5==1
                      |dat_SHARE$ph006d6==1
                      |dat_SHARE$ph006d10==1
                      |dat_SHARE$ph006d11==1
                      |dat_SHARE$ph006d13==1
                      |dat_SHARE$ph006d14==1
                      |dat_SHARE$ph006d15==1
                      |dat_SHARE$ph006d16==1
                      |dat_SHARE$ph006d19==1
                      |dat_SHARE$ph006d20==1)
                     ,1,0)



dat_SHARE$e_num<-(replace(dat_SHARE$ph006d1, dat_SHARE$ph006d1 < 0, 0)
                  +replace(dat_SHARE$ph006d2, dat_SHARE$ph006d2 < 0, 0)
                  +replace(dat_SHARE$ph006d3, dat_SHARE$ph006d3 < 0, 0)
                  +replace(dat_SHARE$ph006d4, dat_SHARE$ph006d4 < 0, 0)
                  +replace(dat_SHARE$ph006d5, dat_SHARE$ph006d5 < 0, 0)
                  +replace(dat_SHARE$ph006d6, dat_SHARE$ph006d6 < 0, 0)
                  +replace(dat_SHARE$ph006d10, dat_SHARE$ph006d10 < 0, 0)
                  +replace(dat_SHARE$ph006d11, dat_SHARE$ph006d11 < 0, 0)
                  +replace(dat_SHARE$ph006d13, dat_SHARE$ph006d13 < 0, 0)
                  +replace(dat_SHARE$ph006d14, dat_SHARE$ph006d14 < 0, 0)
                  +replace(dat_SHARE$ph006d15, dat_SHARE$ph006d15 < 0, 0)
                  +replace(dat_SHARE$ph006d16, dat_SHARE$ph006d16 < 0, 0)
                  +replace(dat_SHARE$ph006d19, dat_SHARE$ph006d19 < 0, 0)
                  +replace(dat_SHARE$ph006d20, dat_SHARE$ph006d20 < 0, 0)                          
)
...
#table(dat_SHARE$e_num) #frequency table of number of chronic diseses per person 

dat_SHARE$f<-ifelse(is.na(dat_SHARE$e),0,1)
#f=1  Response regarding chronic diseases recorded 
#f=0  No record of chronic disease response

dat_SHARE$f2<-ifelse(is.na(dat_SHARE$e)&dat_SHARE$deceased==1,1,0)
...
#table(dat_SHARE$ph005_)
#TABLE
#    -2   -1    0    1 
#     2    2 2243  143

dat_SHARE$g<-ifelse(dat_SHARE$ph005_==3,1,0)
#table(dat_SHARE$g)
...
#list(dat_SHARE$hc602_)
#  value  label
#-2    Refusal
#-1 Don't know

#table(dat_SHARE$hc602_)
#TABLE
...
dat_SHARE$h<-ifelse(dat_SHARE$hc602_>=6,1,0)
...
#list(dat_SHARE$hc014_)
#  value  label
#-2    Refusal
#-1 Don't know

#table(dat_SHARE$hc014_)
#TABLE
...
dat_SHARE$i<-ifelse(dat_SHARE$hc014_>=6,1,0)
...
#list(dat_SHARE$hc013_)
#  value  label
#-2    Refusal
#-1 Don't know

#table(dat_SHARE$hc013_)
#TABLE
#  0   1   2   3   4   5   7   8   9  10 
#  3 284  64  25   6   9   8   3   1  18 

dat_SHARE$j<-ifelse(dat_SHARE$hc013_>=6,1,0)
...
####Left hand average
dat_SHARE$r_ave<-(dat_SHARE$gs008_+dat_SHARE$gs009_)/2
####Right hand average
dat_SHARE$l_ave<-(dat_SHARE$gs006_+dat_SHARE$gs007_)/2


####Dominant hand
dat_SHARE$d_ave<-isTRUE(dat_SHARE$gs004_==2)*dat_SHARE$l_ave+(1-isTRUE(dat_SHARE$gs004_==2))*dat_SHARE$r_ave
...
hands_tested<-2-1*(is.na(dat_SHARE$r_ave)==TRUE)-1*(is.na(dat_SHARE$l_ave)==TRUE)
...
one_hand_l<-one_hand[!is.na(dat_SHARE[one_hand,"l_ave"]>0)]
dat_SHARE[one_hand_l,"d_ave"]<-dat_SHARE[one_hand_l,"l_ave"]
###people who's one hand measurement is the right hand
one_hand_r<-one_hand[!is.na(dat_SHARE[one_hand,"r_ave"]>0)]
dat_SHARE[one_hand_r,"d_ave"]<-dat_SHARE[one_hand_r,"r_ave"]


gender<-as.matrix(dat_SHARE$gender)
age_int<-as.matrix(dat_SHARE$age_int)
d_grip_test_threshhold<-0
nd_grip_test_threshhold<-0

for (i in 1:nrow(dat_SHARE))
{if(age_int[i]<0){d_grip_test_threshhold[i]<-NA}
  else
  {mean_hand_grip<-dat_hand_grip[which(dat_hand_grip$Gender   ==gender[i]
                                       &dat_hand_grip$Age_min <=age_int[i]
                                       &dat_hand_grip$Age_max >=age_int[i]
                                       &dat_hand_grip$Hand    =="D")
                                 ,"Mean"]
  std_hand_grip<-dat_hand_grip[which(dat_hand_grip$Gender   ==gender[i]
                                     &dat_hand_grip$Age_min <=age_int[i]
                                     &dat_hand_grip$Age_max >=age_int[i]
                                     &dat_hand_grip$Hand    =="D")
                               ,"SD_AbsoluteVal"] 
  d_grip_test_threshhold[i]<-as.double(mean_hand_grip)-as.double(std_hand_grip)
  }}
dat_SHARE[,"d_grip_test_threshhold"]<-d_grip_test_threshhold[i]


#table(dat_SHARE$d_grip_test_threshhold)

#d_grip_strength = 1 if dominant hand strong enough
#d_grip_strength = 0 if dominant hand not strong enough
dat_SHARE$d_grip_strength<-(dat_SHARE$d_ave>dat_SHARE$d_grip_test_threshhold)*1+(1-(dat_SHARE$d_ave>dat_SHARE$d_grip_test_threshhold))*0
table(dat_SHARE$d_grip_strength)

####Non-dominant hand
dat_SHARE$nd_ave<-isTRUE(dat_SHARE$gs004_==2)*dat_SHARE$r_ave+(1-isTRUE(dat_SHARE$gs004_==2))*dat_SHARE$l_ave
#label(nd_ave) <- "Average handgrip non-dominant hand"

for (i in 1:nrow(dat_SHARE))
{if(age_int[i]<0){nd_grip_test_threshhold[i]<-NA}
  else
  {mean_hand_grip<-dat_hand_grip[which(dat_hand_grip$Gender   ==gender[i]
                                       &dat_hand_grip$Age_min <=age_int[i]
                                       &dat_hand_grip$Age_max >=age_int[i]
                                       &dat_hand_grip$Hand    =="ND")
                                 ,"Mean"]
  std_hand_grip<-dat_hand_grip[which(dat_hand_grip$Gender   ==gender[i]
                                     &dat_hand_grip$Age_min <=age_int[i]
                                     &dat_hand_grip$Age_max >=age_int[i]
                                     &dat_hand_grip$Hand    =="ND")
                               ,"SD_AbsoluteVal"] 
  nd_grip_test_threshhold[i]<-as.double(mean_hand_grip)-as.double(std_hand_grip)
  }}
dat_SHARE[,"nd_grip_test_threshhold"]<-nd_grip_test_threshhold[i]

#table(dat_SHARE$d_grip_test_threshhold)

#nd_grip_strength = 1 if non-dominant hand strong enough
#nd_grip_strength = 0 if non-dominant hand not strong enough
dat_SHARE$nd_grip_strength<-(dat_SHARE$nd_ave>dat_SHARE$nd_grip_test_threshhold)*1+(1-(dat_SHARE$nd_ave>dat_SHARE$nd_grip_test_threshhold))*0


dat_SHARE$j1<-1*(dat_SHARE$d_grip_strength==0&dat_SHARE$nd_grip_strength==0)
#j1=1 no hand passed the grip strength test
#j1=0 at Least one hand passed grip strength test

#table(dat_SHARE$j1)
#which(dat_SHARE$j1==1)
...
#table(dat_SHARE$ph003_)
#TABLE
#-2   -1    1    2    3    4    5 
# 1    4  230  609 1024  416  111 

dat_SHARE$j2<-ifelse(dat_SHARE$ph003_==5,1,0)
...
#table(dat_SHARE$br015_)
#TABLE
#-2   1   2   3   4 
# 1 299 133  74 246 

dat_SHARE$j3a<-ifelse(dat_SHARE$br015_==3|dat_SHARE$br015_==4,1,0)
...
#table(dat_SHARE$br016_)
#TABLE
#-2   1   2   3   4 
# 1 570  90  31  61 

dat_SHARE$j3b<-ifelse(dat_SHARE$br016_==3|dat_SHARE$br016_==4,1,0)
...
dat_SHARE$j3<-ifelse(dat_SHARE$j3a==1&dat_SHARE$j3b==1,1,0)
...
list(dat_SHARE$cf010_)
#value      label
#-2    Refusal
#-1 Don't know

table(dat_SHARE$cf010_)
#TABLE
...
table(dat_SHARE$ph006d16)
#TABLE
#  -2   -1    0    1 
#   1    2 2359   31 

dat_SHARE$n<-ifelse(dat_SHARE$ph006d16==1&dat_SHARE$interview==1,1,0)
...
dat_SHARE$k<-dat_SHARE$deceased==0
...
dat_SHARE$health_state<-
  
  
  
  table(dat_SHARE$health_state)


lize<-dat_SHARE[which(dat_SHARE$health_state=="lost"),]
